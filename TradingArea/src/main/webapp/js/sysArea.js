layui.use([ 'laypage', 'layer', 'form' ], function() {
	...
	var loadData = function(page) {
		...
		$.ajax({
			url : _basePath+"sys/area/getList.do?page="+page,
			dataType: "json",
	...
	$("#edit").on("click", function() {
		...
		selectObjs.each(function(index) {
			...
			window.location.href = _basePath+'sys/area/editView/'+dataId;
		});
	});
	...
	$("#delete").on("click", function(ele) {
		...
		layer.confirm('确认删除？', {
			...
			// 提交后台删除
			$.ajax({
				url : _basePath + "sys/area/delete.do",
				type: "post",
				data: {"id": dataId},
				dataType : "json",
				success : function(data) {
