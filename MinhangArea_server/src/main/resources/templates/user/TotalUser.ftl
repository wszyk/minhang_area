	...
	<!-- showData :vue数据展示标签ID-->
		<div id='showData'>
			<table>
				<thead>
					<tr>
						<th colspan="22">累计用户报表统计</th>
					</tr>
					<tr>
						<th style="height:10%">ID</th>
						<th style="height:20%">语言</th>
						<th style="height:20%">渠道</th>
						<th style="height:10%">总注册数</th>
						<th style="height:10%">总活跃数</th>
						<th style="height:10%">总付费数</th>
						<th style="height:20%">总付费率</th>
		...
		<script type="text/javascript">
			...
			// 初始化分页vue
			var pagination = createPagination(showData,'../ReportForms/totalUserData');
			pagination.pageSize = 8;
			// 加载数据
			showDataFun(showData,pagination,'../ReportForms/totalUserData');
			function queryNewUser(){
				var langVal = $('#lang').val();
				var channelVal = $('#channel').val();
				showDataFun(showData,pagination,'../ReportForms/totalUserData?channel='+channelVal+'&lang='+langVal);
			}
		</script>
	</div>
	<div style="position:fixed; top:46%; left:0%;">
		<span>
			...
			<select id='dataType' name="dataType">
				<option value="">--请选择--</option>
              	<option value="1">总注册用户</option>
            	<option value="2">总激活用户</option>
            	<option value="3">总付费数</option>	
            	<option value="4">总付费比率</option>
			</select>
		<span>
	</div>
		...
		function initHighcharts() {
			var title = {
				...
				json.legend = legend;
	   			$.ajax({
	   				type:'GET',
	   				url:'../ReportForms/totalUserDataLine?dataType=1',
	   		...
	   		$('#dataType').change(function(){
	   			...
	   			$.ajax({
	   				type:'GET',
	   				url:'../ReportForms/totalUserDataLine?dataType='+dataType,
	   				success:function(data){
	   					var jsonStr = JSON.stringify(data);
	   					var dataObj = eval("("+jsonStr+")");
	      				$.each(dataObj,function(i,dataIndex){
